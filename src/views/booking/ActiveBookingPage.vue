  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <div class="w-full max-s-sm mx-auto mt-6 px-6 flex flex-col justify-center items-center">
        ...
        <!-- tickets -->
        <div v-else-if="!isLoading && activeBookings.length > 0" class="w-full mt-6">
          <TicketDiv v-for="booking in activeBookings" :key="booking.booking_id" :booking="booking" />
        </div>

        <!-- no active booking -->
        <div v-else class="w-full mt-12 flex flex-col justify-center items-center">
          <p class="text-2xl text-blue-800 font-semibold">No Active Bookings</p>
          <p class="mt-2 text-sm text-gray-500 px-3">You haven't made any bus bookings</p>
...
// variables
var activeBookings = ref([])
var isLoading = ref(true)

// functions
async function getActiveBookings(){
  await axios.get('bookings?filter=active')
    .then((response) => {
      activeBookings.value = response.data.bookings;
