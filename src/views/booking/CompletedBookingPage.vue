  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <div class="w-full max-s-sm mx-auto mt-6 px-6 flex flex-col justify-center items-center">
        ...
        <!-- tickets -->
        <div v-else-if="!isLoading && completedBookings.length > 0" class="w-full mt-6">
          <TicketDiv v-for="booking in completedBookings" :key="booking.booking_id" :booking="booking" />
        </div>

        <!-- no active booking -->
        <div v-else class="w-full mt-12 flex flex-col justify-center items-center">
          <p class="text-2xl text-blue-800 font-semibold">No Completed Booking Yet</p>
          <p class="mt-2 text-sm text-gray-500 px-3">You haven't made any travels with us</p>
...
// variables
var completedBookings = ref([])
var isLoading = ref(true)

// functions
async function getCompletedBookings(){
  await axios.get('bookings?filter=completed')
    .then((response) => {
      completedBookings.value = response.data.bookings;
