    ...
    <ion-content :fullscreen="true" >
     <div class="w-full min-h-screen bg-gray-100">
      ...
      <div class="w-full bg-white flex items-center justify-around">
        <router-link :to="{name: 'boarding-points'}" class="w-full text-center">
        ...
        <div class="w-full rounded-lg shadow-sm bg-white">
          ...
          <div v-if="isLoading" class="w-full p-4">
            <div class="w-full flex justify-center items-center gap-4">
                ...
                <p class="text-lg font-semibold text-blue-900">Loading Dropping Points</p>
            </div>
          </div>

          <div v-else class="w-full divide-y-2 devide-gray-100">
            <div v-for="point in droppingPoints" :key="point" v-on:click="selectDroppingPoint(point)" class="flex items-center justify-between gap-4 py-3 px-4 focus:bg-gray-200"> 
              <div>
                <p class="text-sm font-bold">{{ point.dropping_time }}</p>
...
const droppingPoints = ref([])
const selectedBus = ref('')
const selectedBoardingPoint = ref(null)
...
async function getSearchPayload(){
  ...
  selectedBoardingPoint.value = await IonicPreference.getPreference('selected_boarding_point')
...
function selectDroppingPoint(point){
  IonicPreference.setNewPreference('selected_dropping_point', JSON.stringify(point));
  router.push({name: 'passenger-info-page'});
}

async function getDroppingPoints(){
  await axios.post('getDroppingPoints', {
    'boarding': JSON.parse(selectedBoardingPoint.value).place_slug,
  ...
  .then((response) => {
    droppingPoints.value = response.data.dropping_points;
