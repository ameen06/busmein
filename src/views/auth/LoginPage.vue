        ...
        <div class="w-full bg-white max-w-sm rounded-xl shadow-md">
          <div class="w-full px-6 py-8 space-y-4">
            ...
            <div class="w-full rounded-lg border-[2px] border-blue-800 flex justify-center items-center p-0">
              <router-link :to="{name: 'register'}" class="w-full text-blue-800 font-semibold text-center px-4 py-2.5 bg-white rounded-md focus:outline-none focus:bg-blue-900">
                SIGN UP
              </router-link>
            ...
            <div class="w-full space-y-4">
              ...
              <button type="button" v-on:click.prevent="login" class="w-full text-white bg-blue-800 disabled:bg-blue-600 font-semibold rounded-lg px-5 py-4 focus:outline-none uppercase inline-flex items-center justify-center gap-2" :disabled="isLoading">
...
<script setup>
import { IonContent, IonPage, IonToast, onIonViewDidEnter } from "@ionic/vue"
import { ref } from "vue"
...
async function login(){
  ...
  await axios.post('auth/login', {'phone': phone.value})
  .then((response) => {
